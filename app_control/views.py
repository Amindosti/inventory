...
from .serializer import (
    ...
    InventoryGroupSerializer, InventorySerializer, Shop,
    ShopSerializer, Invoice, InvoiceSerializer,
    InventoryWithSumSerializer, ShopWithAmountSerializer,
    InvoiceItem
)
from back.custome_method import IsAuthenticatedCustom
...
class InventoryView(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if keyword:
            search_fields = (
                "code", "created_by__fullname", "group__name",
                "name",
            )
...
class InventoryGroupView(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if keyword:
            search_fields = (
                "created_by__fullname", "group__name",
                "name",
            )
            query = get_query(keyword, search_fields)
...
class InvoiceView(ModelViewSet):
    queryset = Invoice.objects.select_related("created_by",
                                              "shop").prefetch_related("invoice_items")
...
class SalePerformanceView(ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        if not total:
            start_date = query_data.get("start_data", None)
            end_date = query_data.get("end_data", None)
...
class PurchaseView(ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        query = query.aggregate(
            amounts_total=Sum(F('amount') * F('quantity')), total=Sum('quantity')
        )
